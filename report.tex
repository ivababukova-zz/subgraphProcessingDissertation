...
Collisions can occur also if the size of the fingerprint is too small for the particular data. However, making the fingerprint size too big introduces additional overhead by requiring more memory storage space that is not used. The paper does not specify the hash function used or how to decide on the size of bitsets (feature hash tables). 
...
\item Each of the four datasets can be kept in memory memory of a standard laptop without the usage of additional hardware.
...
\centering
        ...
        \begin{tabular}{c|bg|bg|bg|bg|bg|}
            ...
            \hline
        \end{tabular}
        \caption{Number of search nodes taken to solve the SIP instances for each dataset. Blue for SAT and red for UNSAT SIP}
        \label{table:SATUNSATnodes}
    \end{table}

\section{Comparison with Big Data algorithms}
\label{sec:bigDataCompare}
We make a comparison of Light Filters with a selection of ``well-established'' subgraph query processing algorithms, namely CT-Index \cite{ctindex}, gCode \cite{gcode}, Grapes \cite{GRAPES}, tree+$\Delta$ \cite{tree+delta>=graph}, GGSX \cite{graphgrepsx} and gIndex \cite{freqStructBasedIndexing1}. Their evaluation is described in \cite{foteini}. All six algorithms implement the filtering-verification framework with heavy filtering approach using an index structure and run a VF2 \cite{vf2} or similar \gls{sip} algorithm for verification. The work in \cite{foteini} runs experiments using the same datasets and the same machine as us. The comparison is made first with respect to running time and then with respect to filtering strength.
...
\section{Future Work}
This work puts into question the performance of subgraph query processing methods based on the filtering-verification paradigm, mainly because of the dubious quality of the benchmark datasets. This leads to several possible extensions of this work. %Filtering-verification based algorithms could be tested with datasets that are good representatives of the real demand of Big Data in terms of size and complexity.
